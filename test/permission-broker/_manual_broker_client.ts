...
const daemon = new Deno.Command(Deno.execPath(), {
  args: ["run", "--allow-net", "--allow-read", "--allow-write", "--allow-env", "src/permission-broker/daemon-entry.ts"],
  env: {
...
for (
  const [id, perm, value] of [
    [1, "read", path.join(workspace, "a", "f.txt")],
    [2, "read", path.join(project, "src", "secret.ts")],
  ] as const
) {
  await writeJsonlLine(conn, JSON.stringify({ v: 1, pid: 1, id, datetime: "t", permission: perm, value }));
  const resp = await readJsonlLine(conn);
  console.log(id, resp);
}
conn.close();
daemon.kill();
